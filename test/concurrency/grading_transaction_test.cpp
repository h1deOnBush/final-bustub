...
TEST_F(GradingTransactionTest, IntegratedTest) {
  ...
  const Schema *out_delete_schema;
  {
    ...
    auto predicate = MakeComparisonExpression(colA, const1, ComparisonType::Equal);
    out_delete_schema = MakeOutputSchema({{"colA", colA}});
  ...
  std::thread t1([&] {
    ...
    ASSERT_EQ(countA_val, TEST2_SIZE - 1);
    // Should sum from 0 to TEST2_SIZE - 1 except for 1
    ASSERT_EQ(sumA_val, TEST2_SIZE * (TEST2_SIZE - 1) / 2 - 1);
    // Minimum should be 0
